...
def pearson_knn_search(data, ldata,ndata):
    ...
    ldata = jnp.where(filter_ceros==0,0,ldata)
    data = jnp.where(filter_ceros==0,0,data)
    res = ((jnp.multiply(ldata,data)).sum(axis=0) - (jnp.multiply((data).sum(axis=0),(ldata).sum(axis=0)))/ndata)/jnp.multiply(jnp.sqrt(jnp.multiply(data,data).sum(axis=0)-jnp.power((data).sum(axis=0),2)/ndata),(jnp.sqrt(jnp.multiply(ldata,ldata).sum(axis=0)-jnp.power((ldata).sum(axis=0),2)/ndata)))
...
def coseno_knn_search(data, ldata):
    ...
    ldata = jnp.where(filter_ceros==0,0,ldata)
    data = jnp.where(filter_ceros==0,0,data)
    res = jnp.divide((jnp.multiply(ldata,data).sum(axis=0)),(jnp.multiply(jnp.sqrt(jnp.multiply(data,data).sum(axis=0)),jnp.sqrt(jnp.multiply(ldata,ldata).sum(axis=0)))))
    res = jnp.where(jnp.isnan(res),-1,res)
    return res
...
def test():
    ...
    #print(names[0])
    _data = []
    _data_zeros = np.count_nonzero(data.transpose(),axis=0)
    count = 0
    for x in range(len(_data_zeros)):
        if(_data_zeros[x]!=0):
            _data.append(x)
    
    data_nz = (data[_data][:])

    list_knn = knn_search_by_point(data_nz.transpose(),data_nz[0], K,p=2,d=1)
    for k in list_knn[0]:
       print(k[0],'____',k[1])
    #print(regular_data)
